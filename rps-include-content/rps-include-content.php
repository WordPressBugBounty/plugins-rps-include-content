...
/* 	Copyright (C) 2012 - 2023  Red Pixel Studios  (email : support@redpixel.com)
...
class RPS_Include_Content {
	...
	public function cb_include_shortcode( $atts, $content = null ) {
		...
		$disable_hover = ( is_multisite() ) ? get_blog_option( $current_blog_id, '_' . self::PLUGIN_PREFIX . 'disable_hover', '' ) : get_option( '_' . self::PLUGIN_PREFIX . 'disable_hover', '' );
		$private_content = (is_multisite() ) ? get_blog_option( $current_blog_id, '_' . self::PLUGIN_PREFIX . 'private_content', '' ) : get_option( '_' . self::PLUGIN_PREFIX . 'private_content', '' );
		...
		$restricted_by_privacy = false;
		if (
			! $private and
			$the_post->post_status == 'private' and
			(
				( ! current_user_can( 'read_private_posts' ) ) or
				( ! current_user_can( 'read_private_pages' ) and $page !== 0 )
			)
		) {
			$restricted_by_privacy = true;
		}
		 ...
		 */	
		elseif ( $restricted_by_privacy ) :
		
			/**
			 * Display a helpful message about the private content if the user is logged in or not.
			 */
			if ( is_user_logged_in() ) :
				$the_article = sprintf( '<p>%s</p>', __( 'You do not have permission to view this content.', 'rps-include-content' ) );
			else :
				$the_article = sprintf( '<p>%s</p>', __( 'You must be logged in to view this content.', 'rps-include-content' ) );
			endif;
		...
		$the_content .= ( $featured_image and ! $restricted_by_privacy ) ? $this->get_the_post_thumbnail( $blog, $the_post->ID, $featured_image_size, $featured_image_wrap, $featured_image_wrap_class ) : '';
